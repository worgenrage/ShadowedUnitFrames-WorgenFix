...
function ShadowUF:OnInitialize()
	...
	-- Initialize DB
	self.db = LibStub:GetLibrary("AceDB-3.0"):New("ShadowedUFDBWorgenFix", self.defaults, true)
	local clientAPIMode = self.db.global.worgenFix.clientAPIMode
	if( clientAPIMode ~= "auto" and clientAPIMode ~= "modern" and clientAPIMode ~= "legacy" ) then
		self.db.global.worgenFix.clientAPIMode = "auto"
	end
...
function ShadowUF:HideBlizzardFrames()
	if( self.db.profile.hidden.cast and not active_hiddens.cast ) then
		hideBlizzardFrames(true, ShadowUF.API.GetPlayerCastingBarFrame(), PetCastingBarFrame)
	end
	...
	if( self.db.profile.hidden.buffs and not active_hiddens.buffs ) then
		if( ShadowUF.API.IsModernClassicUI() ) then
			hideBlizzardFrames(false, BuffFrame)
		else
