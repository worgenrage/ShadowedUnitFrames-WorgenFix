		...
		args = {
			general = {
				...
				args = {
					...
					worgenFixCompatibility = {
						order = 1.5,
						type = "group",
						inline = true,
						name = L["Client compatibility"],
						args = {
							clientAPIMode = {
								order = 1,
								type = "select",
								name = L["Client API mode"],
								desc = L["Selects which Blizzard UI API generation is preferred. Auto selects Modern Classic UI on interface 11508 or newer. Missing APIs always fall back safely. Reload the UI after changing this option."],
								values = {
									auto = L["Auto"],
									modern = L["Modern Classic UI"],
									legacy = L["Legacy Classic UI"],
								},
								get = function()
									return ShadowUF.db.global.worgenFix.clientAPIMode
								end,
								set = function(_, value)
									ShadowUF.db.global.worgenFix.clientAPIMode = value
								end,
							},
							resolvedMode = {
								order = 2,
								type = "description",
								name = function()
									local mode = ShadowUF.API.GetClientAPIMode()
									return string.format(L["Active API mode: %s"], mode == "modern" and L["Modern Classic UI"] or L["Legacy Classic UI"])
								end,
							},
						},
					},
			...
			project = {
				...
				args = {
					...
					version = {
						order = 7,
						type = "description",
						name = L["Fork version: v4.3.9-classic-worgenfix.10"],
					},
					status = {
						order = 8,
						type = "description",
			...
			links = {
				...
				args = {
					...
					originalSource = {
						order = 2,
						type = "input",
						name = L["Original GitHub repository"],
						get = function() return "https://github.com/Shadowed/ShadowedUnitFrames" end,
						set = false,
						width = "full",
					},
					maintainedSource = {
						order = 3,
						type = "input",
						name = L["Maintained upstream repository"],
						get = function() return "https://github.com/Nevcairiel/ShadowedUnitFrames" end,
						set = false,
						width = "full",
					},
					upstreamRelease = {
						order = 4,
						type = "input",
						name = L["Exact upstream release used by this fork"],
					...
					forkSource = {
						order = 5,
						type = "input",
						name = L["WorgenFix GitHub repository"],
					...
					forkReleases = {
						order = 6,
						type = "input",
						name = L["WorgenFix releases"],
						get = function() return "https://github.com/worgenrage/ShadowedUnitFrames-WorgenFix/releases" end,
						set = false,
						width = "full",
					},
				},
